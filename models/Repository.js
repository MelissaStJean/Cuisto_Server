...
class Repository {
    ...
    bubbleSortAllAuthor(){
        let lgt = this.objectsList.length - 1;
        for (let i = 0; i < lgt ; i++) {
            for (let j = 0; j < lgt; j++) {
                if (this.objectsList[j].UserId > this.objectsList[j + 1].UserId) {
    ...
    bubbleSortAuthor(searchParam){
        let arraySearch = [];
        for(let i = 0 ; i< this.objectsList.length;i++){
            console.log(this.objectsList[i].UserId);
            if(this.objectsList[i].UserId == searchParam.toLowerCase()){
                arraySearch.push(this.objectsList[i]);
            }
        }
        return arraySearch;
    }

    bubbleSortNewsByName(searchParam){
        let arraySearch = [];
        for(let i = 0 ; i < this.objectsList.length; i++){
            if(this.objectsList[i].Title.toLowerCase().includes(searchParam.toLowerCase())){
                arraySearch.push(this.objectsList[i]);
            }
        }
        return arraySearch;
    }
    bubbleSortNewsByDate(){
        let lgt = this.objectsList.length - 1;
        for (let i = 0; i < lgt ; i++) {
            for (let j = 0; j < lgt; j++) {
                if (this.objectsList[j].Created < this.objectsList[j + 1].Created) {
                    let temp = this.objectsList[j];
                    this.objectsList[j] = this.objectsList[j + 1];
                    this.objectsList[j + 1] = temp;
                }
